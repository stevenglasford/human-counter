...
def process_video(video_path):
    ...
    human_totals, bus_totals, car_totals, bike_totals = [], [], [], []
    ...
    while cap.isOpened():
        ...
        human_dets = norfair_detections_from_yolo(results, 0)
        car_dets = norfair_detections_from_yolo(results, 2)
        bus_dets = norfair_detections_from_yolo(results, 5)
        bike_dets = norfair_detections_from_yolo(results, 1)
        ...
        bus_now = any(int(cls) == 5 for cls in results.boxes.cls)
        ...
        if frame_count % fps == 0:
            ...
            human_totals.append(len(human_dets))
            bus_totals.append(len(bus_dets))
            car_totals.append(len(car_dets))
            bike_totals.append(len(bike_dets))
    ...
    for i, row in df.iterrows():
        sec = row["second"]
        light = row["light"]
        if light == "red" and row["red_violation"] and row["bus_waiting"]:
            plt.axvline(x=sec, color='k', linestyle='--', linewidth=2)
            plt.axvline(x=sec, color='r', linestyle='--', linewidth=1.5)
        elif light == "red" and row["red_violation"]:
            plt.axvline(x=sec, color='r', linestyle='--', linewidth=1.5)
        elif light == "red" and row["bus_waiting"]:
            plt.axvline(x=sec, color='k', linestyle='-', linewidth=2)
            plt.axvline(x=sec, color='r', linestyle='-', linewidth=1.5)
        elif light == "red":
            plt.axvline(x=sec, color='r', linestyle='-', linewidth=1.5)
    ...
    plt.plot(df["second"], df["idle_count"], label="Idle", color="brown", linewidth=2)
    plt.plot(df["second"], df["walking_count"], label="Walking", color="blue", linewidth=2)
    plt.plot(df["second"], df["total_count"], label="Total", color="black", linewidth=2)
    plt.plot(df["second"], df["car_speed_mph"], label="Car Speed (mph)", color="orange", linewidth=2)
    plt.plot(df["second"], df["humans"], label="Humans", linestyle=":", color="gray", linewidth=1.5)
    plt.plot(df["second"], df["buses"], label="Buses", linestyle=":", color="red", linewidth=1.5)
    plt.plot(df["second"], df["cars"], label="Cars", linestyle=":", color="green", linewidth=1.5)
    plt.plot(df["second"], df["bikes"], label="Bikes", linestyle=":", color="purple", linewidth=1.5)

    total_humans = sum(df["humans"])
    total_buses = sum(df["buses"])
    total_cars = sum(df["cars"])
    total_bikes = sum(df["bikes"])
    textstr = f"Total Humans: {total_humans}\nTotal Buses: {total_buses}\nTotal Cars: {total_cars}\nTotal Bikes: {total_bikes}"
    plt.gcf().text(0.82, 0.02, textstr, fontsize=10, bbox=dict(facecolor='white', alpha=0.6))
