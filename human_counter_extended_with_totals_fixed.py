...
def process_video(video_path):
    ...
    plt.plot(df["second"].to_numpy(), df["humans"].to_numpy(), label="Humans", linestyle=":", color="gray", linewidth=1.5)
    plt.plot(df["second"].to_numpy(), df["buses"].to_numpy(), label="Buses", linestyle=":", color="red", linewidth=1.5)
    plt.plot(df["second"].to_numpy(), df["cars"].to_numpy(), label="Cars", linestyle=":", color="green", linewidth=1.5)
    plt.plot(df["second"].to_numpy(), df["bikes"].to_numpy(), label="Bikes", linestyle=":", color="purple", linewidth=1.5)

    total_humans = sum(df["humans"].to_numpy())
    total_buses = sum(df["buses"].to_numpy())
    total_cars = sum(df["cars"].to_numpy())
    total_bikes = sum(df["bikes"].to_numpy())
