...
def process_video(video_path):
    from ultralytics import YOLO  # re-import in subprocess
    from norfair import Tracker  # re-import in subprocess
    ...
    plt.title("Human Detection with GPU Acceleration")
    plt.legend()
    plt.grid(True)
...
def run_batch(video_paths):
    with mp.Pool(processes=min(mp.cpu_count(), len(video_paths))) as pool:
