...
def compute_gpx_speed_series(times, coords):
    ...
    for i in range(1, len(times)):
        ...
        timestamp = pd.to_datetime(times[i]).tz_localize("UTC") if pd.to_datetime(times[i]).tzinfo is None else pd.to_datetime(times[i])
...
def match_gpx_to_video(gpx_series, video_start_time, video_duration):
    ...
    for i in range(video_duration):
        t = (video_start_time + timedelta(seconds=i)).astimezone(timezone.utc)
        if t in gpx_series:
...
def process_video(video_path):
    ...
    while cap.isOpened():
        ...
        for t in tracked:
            tid = t.id
            cx, cy = t.estimate[0]
            object_histories.setdefault(tid, []).append((frame_count, cx, cy))

        for tid, history in object_histories.items():
            recent = [(x, y) for f, x, y in history if frame_count - fps <= f <= frame_count]
            if len(recent) >= 2:
                d = sum(np.linalg.norm(np.array(recent[i]) - np.array(recent[i - 1])) for i in range(1, len(recent)))
                avg_speed = d / len(recent)
                adjusted_speed = abs(avg_speed - self_speed)
    ...
    plt.plot(df["second"].to_numpy(), df["idle_count"].to_numpy(), label="Idle", color="brown")
    plt.plot(df["second"].to_numpy(), df["walking_count"].to_numpy(), label="Walking", color="blue")
    plt.plot(df["second"].to_numpy(), df["total_count"].to_numpy(), label="Total", color="black")
    ...
    plt.title("Human Detection (GPU Optimized)")
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python human_counter_gpu.py video1.mp4 video2.mp4")
    else:
        video_files = [f for f in sys.argv[1:] if f.lower().endswith(".mp4")]
